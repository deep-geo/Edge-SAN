...
class TrainingDataset(Dataset):
    def __init__(self, data_dir, image_size=256, mode='train', requires_name=True, point_num=1, mask_num=5):
        ...
        self.image_paths = []
        self.label_paths = []
        for img_path in glob.glob(os.path.join(data_dir, "data", "*.png")):
            basename = os.path.basename(img_path)
            label_path = os.path.join(data_dir, "label", basename)
    ...
    def __getitem__(self, index):
        ...
        num_nuclei = int(original_mask.max())
        indices_nuclei = [i + 1 for i in range(num_nuclei)]
        choices_nuclei = random.choices(indices_nuclei, k=self.mask_num)

        for idx_nuclei in choices_nuclei:
            pre_mask = original_mask.copy()

            # 只考虑当前细胞核编号，其它编号细胞核都按照背景处理
            pre_mask[pre_mask != idx_nuclei] = 0.0
            pre_mask[pre_mask == idx_nuclei] = 1.0
...
if __name__ == "__main__":
    train_dataset = TrainingDataset("fluorescence", image_size=256, mode='train', requires_name=True, point_num=1, mask_num=5)
